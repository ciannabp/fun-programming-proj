...
#Load libraries
pacman::p_load(glue, prettyR, rio, tidyverse, viridis)
...
sleep_models <- by_grade %>%
                mutate(m1 = map(data, ~lm(Depression ~ Insomnia, data = .x)),
                       ...
                       m3 = map(data, ~lm(Depression ~ Insomnia*Sleep_Hours, data = .x)))
sleep_models




#Iterate predictive model by grade
sleep_model <- sleep_tidy %>%
               group_by(Grade) %>%
               nest() %>%
               mutate(model = map(data, ~lm(Depression ~ Insomnia, data = .x))) 
sleep_model
...
#Apply to our original function
sleep_model2 <- sleep_model %>%
...
#Plot our results
ggplot(sleep_tidy, aes(x = Insomnia, y = Depression)) +
       geom_point() +
       stat_smooth(method = "lm", formula = Depression ~ Insomnia, size = 1) +
       theme_minimal()

#Messing around with the models
m1 <- lm(Depression ~ Insomnia, data = sleep_tidy)
m2 <- lm(Depression ~ Insomnia + Sleep_Hours, data = sleep_tidy)
m3 <- lm(Depression ~ Insomnia*Sleep_Hours, data = sleep_tidy)
summary(m1)
summary(m2)
summary(m3)
anova(m1, m2)
